...
impl Dispatcher {
    ...
    pub fn dispatch(&self, packet: &Packet) -> Packet {
        let command_type = packet.data.get("command_type")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        ...
        match self.handlers.get(command_type) {
            Some(handler) => {
                match handler.execute(args) {
                    Ok(output) => Packet::new("response", serde_json::json!({
                        "command_id": packet.id,
                        "status": "success",
                        "data": output,
                    })),
                    Err(e) => Packet::new("response", serde_json::json!({
                        "command_id": packet.id,
                        "status": "error",
                        "data": e.to_string(),
                    })),
            ...
            None => Packet::new("error", serde_json::json!({
